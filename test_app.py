...
class TestCoupon:
    ...
    @pytest.mark.parametrize("value", [-10, -1, 0, -0.01])
    def test_coupon_rejects_non_positive_value(self, client, value):
        """Should reject coupons with zero or negative values."""
        payload = {"name": "bad-coupon", "value": value}
        response = client.post("/api/v1/coupon", json=payload)
        assert response.status_code == 400

    def test_coupon_rejects_non_numeric_value(self, client):
        """Should reject coupons with non-numeric values."""
        payload = {"name": "bad-coupon", "value": "free"}
        response = client.post("/api/v1/coupon", json=payload)
        assert response.status_code == 400
